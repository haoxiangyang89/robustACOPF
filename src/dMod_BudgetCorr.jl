function createS_Budget_Corr(fData::fixedData,uData::Dict{Any,Any},vmax,vmin,θDmax,θDmin,partCorr,Γ = 1,numericOpt = 0,PresolveOpt = -1)
  # include the option to have correlations between buses
  ...
  # for the buses within the group, consolidate the binary variables
  # define the auxiliary variables: y & r
  partSize = length(partCorr);
  @variable(sprob, ypplus[i in 1:partSize], Bin);
  @variable(sprob, rpplus[i in fData.IDList]);
  @variable(sprob, ypminus[i in 1:partSize], Bin);
  @variable(sprob, rpminus[i in fData.IDList]);
  @variable(sprob, yqplus[i in 1:partSize], Bin);
  @variable(sprob, rqplus[i in fData.IDList]);
  @variable(sprob, yqminus[i in 1:partSize], Bin);
  @variable(sprob, rqminus[i in fData.IDList]);

  # find the part each bus is in
  partRev = Dict();
  for i in 1:partSize
    for iBus in partCorr[i]
      partRev[iBus] = i;
    end
  end

  @constraint(sprob, ubRpplus1[i in fData.IDList], rpplus[i] <= ypplus[partRev[i]]);
  @constraint(sprob, lbRpplus1[i in fData.IDList], rpplus[i] >= -ypplus[partRev[i]]);
  @constraint(sprob, ubRpplus2[i in fData.IDList], rpplus[i] <= λpi[i] + (1-ypplus[partRev[i]])*M);
  @constraint(sprob, lbRpplus2[i in fData.IDList], rpplus[i] >= λpi[i] - (1-ypplus[partRev[i]])*M);
  @constraint(sprob, ubRpminus1[i in fData.IDList], rpminus[i] <= ypminus[partRev[i]]);
  @constraint(sprob, lbRpminus1[i in fData.IDList], rpminus[i] >= -ypminus[partRev[i]]);
  @constraint(sprob, ubRpminus2[i in fData.IDList], rpminus[i] <= λpi[i] + (1-ypminus[partRev[i]])*M);
  @constraint(sprob, lbRpminus2[i in fData.IDList], rpminus[i] >= λpi[i] - (1-ypminus[partRev[i]])*M);
  @constraint(sprob, ubRqplus1[i in fData.IDList], rqplus[i] <= yqplus[partRev[i]]);
  @constraint(sprob, lbRqplus1[i in fData.IDList], rqplus[i] >= -yqplus[partRev[i]]);
  @constraint(sprob, ubRqplus2[i in fData.IDList], rqplus[i] <= λqi[i] + (1-yqplus[partRev[i]])*M);
  @constraint(sprob, lbRqplus2[i in fData.IDList], rqplus[i] >= λqi[i] - (1-yqplus[partRev[i]])*M);
  @constraint(sprob, ubRqminus1[i in fData.IDList], rqminus[i] <= yqminus[partRev[i]]);
  @constraint(sprob, lbRqminus1[i in fData.IDList], rqminus[i] >= -yqminus[partRev[i]]);
  @constraint(sprob, ubRqminus2[i in fData.IDList], rqminus[i] <= λqi[i] + (1-yqminus[partRev[i]])*M);
  @constraint(sprob, lbRqminus2[i in fData.IDList], rqminus[i] >= λqi[i] - (1-yqminus[partRev[i]])*M);
  @constraint(sprob, oneVertexp[i in 1:partSize], ypplus[i]+ypminus[i] <= 1);
  @constraint(sprob, oneVertexq[i in 1:partSize], yqplus[i]+yqminus[i] <= 1);

  @constraint(sprob,budget, sum(ypplus[i]+ypminus[i] for i in 1:partSize) <= Γ);
  ...
  @variable(sprob, λops1[i in fData.IDList]);
  @variable(sprob, λoqs1[i in fData.IDList]);
  @variable(sprob, λops2[i in fData.IDList]);
  @variable(sprob, λoqs2[i in fData.IDList]);
  ...
  @constraint(sprob,levelop1[i in fData.IDList], λops[i] == conWPpar2[i]*λop1[i]);
  @constraint(sprob,leveloq1[i in fData.IDList], λoqs[i] == conWQpar2[i]*λoq1[i]);
  @constraint(sprob,levelop2[i in fData.IDList], λops2[i] == conWPpar2[i]*λop2[i]);
  @constraint(sprob,leveloq2[i in fData.IDList], λoqs2[i] == conWQpar2[i]*λoq2[i]);
  ...
  @constraint(sprob, ubRopplus1[i in fData.IDList], ropplus[i] <= ypplus[partRev[i]]);
  @constraint(sprob, lbRopplus1[i in fData.IDList], ropplus[i] >= -ypplus[partRev[i]]);
  @constraint(sprob, ubRopplus2[i in fData.IDList], ropplus[i] <= λops2[i] + (1-ypplus[partRev[i]])*M1);
  @constraint(sprob, lbRopplus2[i in fData.IDList], ropplus[i] >= λops2[i] - (1-ypmus[partRev[i]])*M1);
  @constraint(sprob, ubRopminus1[i in fData.IDList], ropminus[i] <= ypminus[partRev[i]]);
  @constraint(sprob, lbRopminus1[i in fData.IDList], ropminus[i] >= -ypminus[partRev[i]]);
  @constraint(sprob, ubRopminus2[i in fData.IDList], ropminus[i] <= λops2[i] + (1-ypminus[partRev[i]])*M1);
  @constraint(sprob, lbRopminus2[i in fData.IDList], ropminus[i] >= λops2[i] - (1-ypminus[partRev[i]])*M1);
  ...
  @constraint(sprob, ubRoqplus2[i in fData.IDList], roqplus[i] <= λoqs2[i] + (1-yqplus[i])*M1);
  @constraint(sprob, lbRoqplus2[i in fData.IDList], roqplus[i] >= λoqs2[i] - (1-yqplus[i])*M1);
  @constraint(sprob, ubRoqminus1[i in fData.IDList], roqminus[i] <= yqminus[partRev[i]]);
  @constraint(sprob, lbRoqminus1[i in fData.IDList], roqminus[i] >= -yqminus[partRev[i]]);
  @constraint(sprob, ubRoqminus2[i in fData.IDList], roqminus[i] <= λoqs2[i] + (1-yqminus[i])*M1);
  @constraint(sprob, lbRoqminus2[i in fData.IDList], roqminus[i] >= λoqs2[i] - (1-yqminus[i])*M1);

  @expression(sprob,restTerm,-sum(fData.rateA[k]*ν1[k] for k in fData.brList if !(fData.rateA[k] == Inf)) -
              sum(rpplus[i]*(uData[i].GPmax - uData[i].GP0) + rpminus[i]*(-uData[i].GP0 + uData[i].GPmin) for i in fData.IDList) -
              sum(rqplus[i]*(uData[i].GQmax - uData[i].GQ0) + rqminus[i]*(-uData[i].GQ0 + uData[i].GQmin) for i in fData.IDList) -
              sum(uData[i].GP0*conGPpar1[i]*λpis[i] + uData[i].GQ0*conGQpar1[i]*λqis[i] - μ4[i,2]/4 + ν4[i]/4 + λvu[i]*vmax[i] + λvl[i]*vmin[i] +
              λops1[i]*conWPpar1[i]*uData[i].WPmax + λoqs1[i]*conWQpar1[i]*uData[i].WQmax - vmax[i]*vmin[i]*λv[i] for i in fData.IDList) -
              sum(conWPpar1[i]*(λops2[i]*(uData[i].WPmax + uData[i].RESP0)) for i in fData.IDList) - sum(conWPpar1[i]*(ropplus[i]*(uData[i].RESPmax - uData[i].RESP0) + ropminus[i]*(uData[i].RESPmin - uData[i].RESP0)) for i in fData.IDList if uData[i].RESP0 > 0) -
              sum(conWQpar1[i]*(λoqs2[i]*(uData[i].WQmax + uData[i].RESQ0)) for i in fData.IDList) - sum(conWPpar1[i]*(roqplus[i]*(uData[i].RESQmax - uData[i].RESQ0) + roqminus[i]*(uData[i].RESQmin - uData[i].RESQ0)) for i in fData.IDList if uData[i].RESQ0 > 0) -
...
function createS_Budget_CorrB(fData::fixedData,uData::Dict{Any,Any},vmax,vmin,θDmax,θDmin,partCorr,Γ = 1,numericOpt = 0,PresolveOpt = -1)
  # include the option to have correlations between buses
  # initialize the sub problem
  # obtian θu from the θDmax and θDmin
  θu = Dict();
  ...
  @constraint(sprob, ubRpplus1[i in fData.IDList;!(i in rmListP)], rpplus[i] <= ypplus[partRev[i]]);
  @constraint(sprob, lbRpplus1[i in fData.IDList;!(i in rmListP)], rpplus[i] >= -ypplus[partRev[i]]);
  @constraint(sprob, ubRpplus2[i in fData.IDList;!(i in rmListP)], rpplus[i] <= λpi[i] + (1-ypplus[partRev[i]])*M);
  @constraint(sprob, lbRpplus2[i in fData.IDList;!(i in rmListP)], rpplus[i] >= λpi[i] - (1-ypplus[partRev[i]])*M);
  @constraint(sprob, ubRpminus1[i in fData.IDList;!(i in rmListP)], rpminus[i] <= ypminus[partRev[i]]);
  @constraint(sprob, lbRpminus1[i in fData.IDList;!(i in rmListP)], rpminus[i] >= -ypminus[partRev[i]]);
  @constraint(sprob, ubRpminus2[i in fData.IDList;!(i in rmListP)], rpminus[i] <= λpi[i] + (1-ypminus[partRev[i]])*M);
  @constraint(sprob, lbRpminus2[i in fData.IDList;!(i in rmListP)], rpminus[i] >= λpi[i] - (1-ypminus[partRev[i]])*M);
  @constraint(sprob, ubRqplus1[i in fData.IDList;!(i in rmListQ)], rqplus[i] <= ypplus[partRev[i]]);
  @constraint(sprob, lbRqplus1[i in fData.IDList;!(i in rmListQ)], rqplus[i] >= -ypplus[partRev[i]]);
  @constraint(sprob, ubRqplus2[i in fData.IDList;!(i in rmListQ)], rqplus[i] <= λqi[i] + (1-ypplus[partRev[i]])*M);
  @constraint(sprob, lbRqplus2[i in fData.IDList;!(i in rmListQ)], rqplus[i] >= λqi[i] - (1-ypplus[partRev[i]])*M);
  @constraint(sprob, ubRqminus1[i in fData.IDList;!(i in rmListQ)], rqminus[i] <= ypminus[partRev[i]]);
  @constraint(sprob, lbRqminus1[i in fData.IDList;!(i in rmListQ)], rqminus[i] >= -ypminus[partRev[i]]);
  @constraint(sprob, ubRqminus2[i in fData.IDList;!(i in rmListQ)], rqminus[i] <= λqi[i] + (1-ypminus[partRev[i]])*M);
  @constraint(sprob, lbRqminus2[i in fData.IDList;!(i in rmListQ)], rqminus[i] >= λqi[i] - (1-ypminus[partRev[i]])*M);
  @constraint(sprob, oneVertexp[i in 1:partSize], ypplus[i]+ypminus[i] <= 1);
  ...
  @constraint(sprob, ubRopplus1[i in fData.IDList], ropplus[i] <= ypplus[partRev[i]]);
  @constraint(sprob, lbRopplus1[i in fData.IDList], ropplus[i] >= -ypplus[partRev[i]]);
  @constraint(sprob, ubRopplus2[i in fData.IDList], ropplus[i] <= λop2[i] + (1-ypplus[partRev[i]])*M1);
  @constraint(sprob, lbRopplus2[i in fData.IDList], ropplus[i] >= λop2[i] - (1-ypplus[partRev[i]])*M1);
  @constraint(sprob, ubRopminus1[i in fData.IDList], ropminus[i] <= ypminus[partRev[i]]);
  @constraint(sprob, lbRopminus1[i in fData.IDList], ropminus[i] >= -ypminus[partRev[i]]);
  @constraint(sprob, ubRopminus2[i in fData.IDList], ropminus[i] <= λop2[i] + (1-ypminus[partRev[i]])*M1);
  @constraint(sprob, lbRopminus2[i in fData.IDList], ropminus[i] >= λop2[i] - (1-ypminus[partRev[i]])*M1);
  @constraint(sprob, ubRoqplus1[i in fData.IDList], roqplus[i] <= ypplus[partRev[i]]);
  @constraint(sprob, lbRoqplus1[i in fData.IDList], roqplus[i] >= -ypplus[partRev[i]]);
  @constraint(sprob, ubRoqplus2[i in fData.IDList], roqplus[i] <= λoq2[i] + (1-ypplus[partRev[i]])*M1);
  @constraint(sprob, lbRoqplus2[i in fData.IDList], roqplus[i] >= λoq2[i] - (1-ypplus[partRev[i]])*M1);
  @constraint(sprob, ubRoqminus1[i in fData.IDList], roqminus[i] <= ypminus[partRev[i]]);
  @constraint(sprob, lbRoqminus1[i in fData.IDList], roqminus[i] >= -ypminus[partRev[i]]);
  @constraint(sprob, ubRoqminus2[i in fData.IDList], roqminus[i] <= λoq2[i] + (1-ypminus[partRev[i]])*M1);
  @constraint(sprob, lbRoqminus2[i in fData.IDList], roqminus[i] >= λoq2[i] - (1-ypminus[partRev[i]])*M1);
