function solveProcess_Budget_Corr(fData::fixedData,uData::Dict{Any,Any},vmax,vmin,θDmax,θDmin,partCorr,Γ = 1,ϵ = 1e-4,nT = 10,numericOpt = 0,PresolveOpt = -1)
  # precalculate the partition correspondence
  partRev = Dict();
  ...
  sp,spobjo = createS_Budget_Corr(fData,uData,vmax,vmin,θDmax,θDmin,partCorr,Γ,numericOpt,PresolveOpt);
...
#  sp.solver = GurobiSolver(MIPGap = 0.04,MIQCPMethod = 0);
  ...
  for i in fData.IDList
    ...
    if !(i in rmListQ)
      bestyq[i] = round(getvalue(sp[:yqplus][partRev[i]])) - round(getvalue(sp[:yqminus][partRev[i]]));
    else
      bestyq[i] = 0;
    end
  end
...
function solveProcess_Budget_CorrB(fData::fixedData,uData::Dict{Any,Any},vmax,vmin,θDmax,θDmin,partCorr,Γ = 1,ϵ = 1e-4,nT = 10,numericOpt = 0,PresolveOpt = -1)
  partRev = Dict();
  for part in 1:length(partCorr)
    for iBus in partCorr[part]
      partRev[iBus] = part;
  ...
  sp,spobjo = createS_Budget_CorrB(fData,uData,vmax,vmin,θDmax,θDmin,partCorr,Γ,numericOpt,PresolveOpt);
  # number of cuts so far and the set of cut coefficients
  nC = 0;
  C = [];
